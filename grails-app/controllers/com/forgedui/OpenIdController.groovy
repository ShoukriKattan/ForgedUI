...
import com.forgedui.SecUserSecRole
...
//		}
		...
		String email;
		String password="devpassABC1234";
		...
		password="devpass12372637236";
		
		if (!createNewAccount(email, password, openId)) {
			//return [command: command, openId: openId]
		}
	...
	private boolean createNewAccount(String username, String password, String openId) {
		boolean created = SecUser.withTransaction { status ->
			

			def config = SpringSecurityUtils.securityConfig

			password = springSecurityService.encodePassword(password)
			def user = new SecUser(username: username, password: password, enabled: true,email:username)
